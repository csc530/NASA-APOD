...
/**
 * To change scenes on the same stage or change and effect the scene or stage
 */
public class Transition{
	/**
	 * <p>
	 * This is used to change scenes with a controller that {@code implements} {@link InitializableAPOD}. Upon change scenes the {@link
	 * InitializableAPOD#initializeAPOD(APOD)} method will be called to set up the given {@link APOD} immediately
	 * </p>
	 * Will change the scene within the same stage if there is an error loading the scene the application will reload the current scene as is
	 *
	 ...
	 */
	public static void addStyle(Node node) throws NullPointerException{
		...
		root.getStylesheets().add(Objects.requireNonNull(Main.class.getResource("styles/style.css")).toString());
	}
}
